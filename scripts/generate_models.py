...
import_value = """from sqlalchemy import Column
...
constants.COLUMN = constants.COLUMN
constants.HEADER_DECL = """
db_url = '%s'
...
def format_model(s):
    s = re.sub(r'\(\s*display_width\s*=\s*\d+\s*\)','',s)
    s = re.sub(r'DATETIME\(\)', 'TIMESTAMP', s)
    s = re.sub(r'TIMESTAMP\(\)', 'TIMESTAMP', s)
    s = re.sub(r',\s*default\s*=\s*CURRENT_TIMESTAMP','',s)
    s = re.sub(r'BIGINT','INTEGER',s)
    return s
...
def write_schema_file(factory, schema_file, name='default'):
    ...
    with open(schema_file, 'wb') as fh:
        fh.write("#-*- coding: utf-8 -*-\n")
        fh.write('\n')
        fh.write(import_value + '\n')
        fh.write('\n')
        fh.write( format_model(repr(factory)) )
        fh.write('\n')
...
if __name__ == '__main__':
    ...
    db_type = re.sub(r'[^\w].*', '', app.config['SQLALCHEMY_DATABASE_URI'])
    import_value = import_value.replace('db_type', db_type)
